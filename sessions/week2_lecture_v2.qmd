...
date-as-string: "13th October 2026"
from: markdown+emoji
format:
  revealjs: 
...
## Last week
**Exploratory data analysis, part 1 — *what have I got?***

- **Our dataset** — DfE school performance in England, 2022/23
- **Data types** — Stevens' four levels, then the finer statistical types
- **Descriptive statistics: shape** — distributions, histograms, boxplots
- **Descriptive statistics: middle and spread** — mean, median, variance, standard deviation
- **Making things comparable** — counts, rates and standard scores
- **Special values** — missing data and outliers

::: {.notes}
Adam's lecture 1 now goes beyond Stevens: nominal / ordinal / interval / ratio
are still there, but so is the finer taxonomy (binary, categorical, ordinal,
binomial, count, real-valued additive, real-valued multiplicative) and the
table linking each type to a distribution and a model. The
numerical-vs-categorical framing was dropped this year, so do not lean on it.

Things lecture 1 deliberately left for us to make precise:
- it used "roughly normal" and the two-thirds rule without defining the Normal
- it put absence on a log axis and said "there is a reason, Bea will explain"
- it changed the population by filtering, and flagged bias as our territory
:::


# This week

## *What lies behind it?* {.smaller}

Last week was **descriptive** — statements about the schools actually in front
of us. This week we go underneath that.

::: {.incremental}
-   **The shapes have names.** Last week's histograms were pictures of data.
    This week they become **probability distributions** — objects with
    parameters, that you can compute and predict with.
-   **Some data misbehaves, and can be fixed.** Exponentials and logarithms,
    and why a logarithmic axis straightened out last week's absence data.
-   **Your data was never the whole story.** Last week we changed the
    population on purpose, by filtering. This week: the bias that was already
    there before anyone touched it.
:::
...
## When descriptive statistics aren't enough
Last week we talked about descriptive statistics - but what about when this doesn't tell the whole story? 
...
[*example*: foot length]{style="color:#abc766"}
[*example*: the number of minutes between tube stops]{style="color:#abc766"}

**Discrete data**
Measurable data which can take seperate, countable values. 

[*example*: shoe size]{style="color:#abc766"} 
[*example*: the number of tube arrivals in an hour]{style="color:#abc766"}
...
- continusous distirbutions - have probability density functions (PDFs)
- discrete distributions - have probability mass functions (PMFs). 
...
::: {.notes}
Replaces the bare "we've covered" list. This is the same table as the "Four distributions, four questions" slide with more columns.
:::
...
**This is what happened last week.** Adam put *% of sessions missed* on a
logarithmic axis and the skew nearly vanished. Same move — he changed the
spacing of the axis, we are changing the values themselves, and both do the
same job. That variable is **log-normal**, and now you know what that means.
:::
...
The callback matters: lecture 1's "real-valued multiplicative" slide explicitly
says "there is a reason this axis works and Bea will explain it next week".
This is that payoff - worth saying out loud so the two lectures visibly join up.
:::

# Representative data
**Before we try to describe the data, it's important to know where the data came from.**

## You have already done this once {.smaller}

Last week, to get a fair comparison, Adam filtered the schools data down to
mainstream secondaries — dropping the special schools and the independent
schools. The headline average moved.

::: {.incremental}
-   That was **deliberate**. You could see exactly what was removed, and why,
    and what it did to the number
-   The lecture called it what it was: *"we have just changed what we are
    studying"*
-   It also turned out the **missing** values were not random either — `SUPP`
    appears when a school is small, so dropping incomplete rows quietly
    deletes small schools
:::

. . .

**So far so honest.** The harder question is the one nobody gets to watch: what
was already missing, or already skewed, *before the dataset reached you?*
