...
- A mean of
  `r sprintf("%.0f", mean(schools_mainstream$TOTPUPS, na.rm = TRUE))`
  pupils is meaningful; a school with 1,204.7 pupils is not
- Distribution: **Poisson** or **negative binomial**, not normal

. . .

- But look at the bottom of that chart. **Disadvantaged pupils** (median
  `r sprintf("%.0f", median(schools_mainstream$TFSM6CLA1A, na.rm = TRUE))`)
  is squashed into nothing next to **boys on roll** (median
  `r sprintf("%.0f", median(schools_mainstream$NUMBOYS, na.rm = TRUE))`)
- They are the *same data type*, on the *same axis*, and still not
  comparable
...
::: {style="background-color:#2e6260; color:white; padding:10px; border-radius:5px; font-size:0.85em;"}
**Weeks 9 and 10** standardise every variable before PCA
and clustering — otherwise "pupils on roll", with a standard deviation
of 398, would drown out "% sessions missed", with a standard deviation
of 2.6.
:::
