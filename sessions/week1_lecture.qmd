...
## How skew is calculated {.smaller}
...
Squaring throws the sign away. Variance tells you **how far** values sit from
the mean, never **which side** they sit on.
:::

::: {.column width="52%"}
**Skew** uses *cubed* deviations:

$$g_1 = \frac{\frac{1}{n}\sum (y_i - \bar{y})^3}{\sigma^3}$$

Cubing **keeps** the sign — a value below the mean stays negative.
:::
:::::
...
::: {.incremental}
- If the large deviations sit mostly to the **right**, the cubes sum to a positive number
- Dividing by $\sigma^3$ cancels the units, so skew is comparable across variables
- Rough guide: below 0.5 is near-symmetric, above 1 is strongly skewed
:::

::: {.notes}
Worth linking back explicitly: we said earlier that squaring makes outliers
count heavily in the variance. Cubing makes them count even more heavily, and
lets them pull in a direction. Skew is essentially "which tail is doing the
damage".

Small print if anyone asks: R's sd() divides by n-1 while the textbook
population formula divides by n, so the figure on the previous slide differs
from the strict definition in about the fourth decimal place.
:::
...
# Same data, same units on the axis - only the SCALE is logged. ggplot applies
# a position scale transformation before the stat, so the bins are computed in
# log space, giving exactly the shape of a histogram of log(PERCTOT).
...
- Both panels show the same percentages. Only the **spacing of the axis** changes: on the right, equal distances mean equal *multiples* rather than equal *amounts*
- On that scale the distribution is very nearly symmetric
